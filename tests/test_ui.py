...
def test_palette_layout_has_17_elements_then_3_tools() -> None:
    ...
    # 17 real element swatches (EMPTY excluded) + 3 utility tools.
    assert len(elements) == len(ElementId) - 1 == 17
...
def test_palette_resolves_new_elements_and_fits_min_window() -> None:
    """Phase 03 added STEAM/ICE/LAVA/GLASS swatches; the acid/base pair added
    ACID/BASE; oil adds OIL; gunpowder adds GUNPOWDER; dry ice adds DRY_ICE;
    liquid nitrogen adds LN2, growing the row into 20 items (17 elements + 3
    tools) separated by a group gap.

    The new elements still resolve via ``item_at`` at their center pixel, and
    the whole 20-item + group-gap row fits inside ``MIN_WINDOW_W`` (bumped to
    584 = 146 cols so the wider gap-separated palette still fits at the minimum
    window size).
    """
    ...
    # The full 20-item + group-gap row fits within MIN_WINDOW_W at the min size.
    last = ui.items[-1]
    assert last.x + last.w + PALETTE_MARGIN <= MIN_WINDOW_W
    # And the documented math: 20 items, 19 inter-item paddings, 1 group gap,
    # 2 outer margins.
    assert (
        20 * PALETTE_SWATCH
        + 19 * PALETTE_PADDING
